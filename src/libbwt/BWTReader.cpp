...
void BWTReaderBinary::read(bwt* pRLBWT) {
    BWFlag flag;
    size_t numStrings;
    readHeader(numStrings, pRLBWT->m_rlString.m_numSymbols, flag);
...
void BWTReaderBinary::readRuns(std::vector<RLUnit>& out, size_t numRuns) {
    out.resize(numRuns);
...
char BWTReaderBinary::readBWChar() {
    ...
    if(m_currRun.isEmpty()) {
        ...
        if(m_numRunsRead == m_numRunsOnDisk) return '\n';
